...
@Entity(tableName = "keranjang", foreignKeys = [
    ForeignKey(entity = KasirEntity::class, parentColumns = ["id"], childColumns = ["kasir_id"], onDelete = ForeignKey.RESTRICT),
], indices = [Index("kasir_id"), Index(value = ["status", "diperbarui_pada"])] // Composite index
data class KeranjangEntity(
    ...
    @ColumnInfo(name = "ditahan_pada") val ditahanPada: Long?,
    @ColumnInfo(name = "diselesaikan_pada") val diselesaikanPada: Long?,
